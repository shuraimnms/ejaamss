...
export const Route = createFileRoute("/conferences/past")({
  head: () => ({
    meta: [
      { title: "Past Conferences (2015 – 2026) — EJAAMSS" },
      { name: "description", content: "Archive of every EJAAMSS international conference and its published proceedings, from 2015 to 2026." },
      { property: "og:title", content: "Past Conferences — EJAAMSS" },
      { property: "og:description", content: "Year-wise tree of EJAAMSS conferences, venues, and conference proceedings volumes." },
    ],
...
const THEMES = [
  "Contemporary Issues in Commerce & Management",
...
function build(): YearBlock[] {
  ...
  for (let year = 2015; year <= 2026; year++) {
    ...
    const conferences: Conf[] = Array.from({ length: count }, (_, i) => ({
      n: i + 1,
      title: `EJAAMSS International Conference on ${THEMES[(i + year) % THEMES.length]}`,
  ...
  return (
    <SiteLayout>
      <PageHero
        ...
        intro="A complete archive of EJAAMSS international conferences, symposia, and their published proceedings — organised year-wise from the journal's launch year."
      />
        ...
        <div className="grid gap-6 p-6 lg:grid-cols-[minmax(0,3fr)_minmax(0,2fr)]">
          ...
          <div className="rounded-lg border border-border bg-secondary/40 p-5">
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              Selected papers from each edition undergo double-blind peer review and are published as an EJAAMSS proceedings volume with individual DOIs.
            </p>
          </div>
        </div>
      ) : null}
