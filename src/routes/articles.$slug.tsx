...
export const Route = createFileRoute("/articles/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.title} — EJAAMSS` },
...
function Article() {
  ...
  const apa = `${(a.authors as string[]).join(", ")} (${issue?.year ?? new Date(a.published_at ?? Date.now()).getFullYear()}). ${a.title}. International Journal of Academic Research in Commerce & Management, ${issue?.volume ?? "-"}(${issue?.number ?? "-"}), ${a.page_start ?? "-"}–${a.page_end ?? "-"}.${a.doi ? " https://doi.org/" + a.doi : ""}`;

  const bibtex = `@article{ejaamss${issue?.year ?? ""}${slug.split("-")[0]},
  title = {${a.title}},
  author = {${(a.authors as string[]).join(" and ")}},
  journal = {International Journal of Academic Research in Commerce & Management},
  volume = {${issue?.volume ?? ""}},
  number = {${issue?.number ?? ""}},
