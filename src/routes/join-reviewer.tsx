...
export const Route = createFileRoute("/join-reviewer")({
  head: () => ({
    meta: [
      { title: "Become a Peer Reviewer — EJAAMSS" },
      { name: "description", content: "Apply to join the EJAAMSS standing peer-review panel. Recognised contributions, reviewer certificates, and priority consideration for editorial roles." },
      { property: "og:title", content: "Join as Reviewer — EJAAMSS" },
      { property: "og:description", content: "Apply to review for EJAAMSS." },
    ],
...
const BENEFITS = [
  { icon: Award, title: "Reviewer certificate", body: "A verifiable EJAAMSS-YYYY-#### certificate for every completed review, publicly verifiable on our /verify page." },
...
const CRITERIA = [
  "Doctoral degree (or equivalent research standing) in commerce, management, or an adjacent field.",
  "At least two peer-reviewed publications as first or corresponding author.",
  "Demonstrable expertise in one or more EJAAMSS subject areas.",
...
const FAQS = [
  ...
  { q: "How is my identity protected?", a: "EJAAMSS uses double-blind review. Author identity is stripped from manuscripts before you receive them, and your identity is never shared with the authors." },
  ...
  return (
    <SiteLayout>
      <PageHero
        ...
        title="Become an EJAAMSS peer reviewer"
        intro="Help shape the scholarly record in commerce and management. Rigorous, timely, double-blind peer review is the heart of what we do."
      />
              ...
              <form onSubmit={submit} className="rounded-2xl border border-border bg-card p-6 lg:p-8">
                ...
                <div className="mt-4">
                  <label className="block">
                    <div className="mb-1 text-xs uppercase tracking-wider text-muted-foreground">Why do you want to review for EJAAMSS?</div>
                    <textarea
                      rows={5}
                      value={f.motivation}
